...
select gender, sum(purchase_amount)as renvene from customer group by gender;
...
select item_purchased, round(avg(review_rating::numeric),2) as "average_review_rating" from customer group by item_purchased 
order by avg(review_rating) desc limit 5;

-- Q4. Compare the average Purchase Amounts between Standard and Express Shipping.
select shipping_type,round(avg(purchase_amount),2) from customer where shipping_type in 
...
select subscription_status,count(customer_id)as total_customer,round(avg(purchase_amount),2) as "avg_spend",sum(purchase_amount) as"total_revenue"
from customer where subscription_status in('Yes','No') group by subscription_status order by total_revenue, avg_spend desc;
